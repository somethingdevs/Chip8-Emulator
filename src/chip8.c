...
void chip8_load(struct chip8 *chip8, const char *buf, size_t size)
{
    ...
    chip8->registers.PC = CHIP8_PROGRAM_LOAD_ADDRESS; //Putting 0x200 in PC since thats where it starts from
}
...
static void chip8_exec_extended_eight(struct chip8 *chip8, unsigned short opcode)
{
    ...
    // 8xy1 - OR Vx, Vy. Performs bitwise OR on Vx and Vy stores result in Vx
    case 0x01:
        chip8->registers.V[x] = chip8->registers.V[x] | chip8->registers.V[y];
        break;

    // 8xy2 - AND Vx, Vy. Performs bitwise AND on Vx and Vy stores result in Vx
    case 0x02:
        chip8->registers.V[x] = chip8->registers.V[x] & chip8->registers.V[y];
        break;

    // 8xy3 - XOR Vx, Vy. Performs bitwise XOR on Vx and Vy stores result in Vx
    case 0x03:
        chip8->registers.V[x] = chip8->registers.V[x] ^ chip8->registers.V[y];
        break;
...
static char chip8_wait_for_key_press(struct chip8 *chip8)
{
    ...
    while (SDL_WaitEvent(&event))
    {
        ...
        char chip8_key = chip8_keyboard_map(&chip8->keyboard, c); // error might occur here
        if (chip8_key != -1)
        {
            return chip8_key;
        }
...
static void chip8_exec_extended_F(struct chip8 *chip8, unsigned short opcode)
{
    ...
    case 0x29:
        ...
        //fx33 - LD B, Vx. Takes decimal value of Vx, places hundereds digits in location I, tens digit
        //at location I+1, and the units digits in location I+2
    case 0x33:
    {
...
static void chip8_exec_extended(struct chip8 *chip8, unsigned short opcode)
{
    ...
    {
        // JP address, 1nnn Jump to location nnn's
    case 0x1000:
        chip8->registers.PC = nnn;
        break;

    // CALL address, 2nnn Call subroutine at location nnn
    case 0x2000:
    ...
    case 0x4000:
        ...
        //5xy0 - SE, Vx, Vy, skip the next instrcutionif Vx = Vy
    case 0x5000:
        if (chip8->registers.V[x] == chip8->registers.V[y])
        {
            chip8->registers.PC += 2;
    ...
    // 8xyF - Big ass list, look at the above function
    case 0x8000:
    ...
    // Dxyn - DRW Vx, Vy, nibble. Draw sprite to the screen
    case 0xD000:
    {
        const char *sprite = (const char *)&chip8->memory.memory[chip8->registers.I];
        chip8->registers.V[0x0f] = chip8_screen_draw_sprite(&chip8->screen, chip8->registers.V[x], chip8->registers.V[x], sprite, n);
    }
    break;
